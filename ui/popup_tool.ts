...
export type DetectionStrategy = "name" | "hash" | "area";
export type InsertMode = "append" | "keep-index";
...
function parseInlineArray(body: string): unknown[] {
...
export function parseYAML(text: string): unknown {
  ...
  for (let index = 0; index < filteredLines.length; index += 1) {
    ...
    if (frame.pending && indent > frame.pending.indent) {
      if (trimmed.startsWith("-")) {
        const arr: unknown[] = [];
        (frame.container as Record<string, unknown>)[frame.pending.key] = arr;
        const arrayFrame: StackFrame = {
          type: "array",
          container: arr,
        ...
        const obj: Record<string, unknown> = {};
        (frame.container as Record<string, unknown>)[frame.pending.key] = obj;
        const objectFrame: StackFrame = {
          type: "object",
          container: obj,
          indent: frame.pending.indent,
          pending: null,
    ...
    if (trimmed.startsWith("-")) {
      ...
      if (content === "") {
        const obj: Record<string, unknown> = {};
        (frame.container as unknown[]).push(obj);
      ...
      if (content.includes(":")) {
        ...
        (frame.container as unknown[]).push(parseScalar(content));
      }
      continue;
    }
    ...
    if (valuePart === "") {
      ...
      (frame.container as Record<string, unknown>)[key] = parseScalar(valuePart);
    }
  }
  ...
  while (stack.length > 1) {
    const top = stack.pop();
    if (top && top.pending) {
      (stack[stack.length - 1].container as Record<string, unknown>)[top.pending.key] = {};
    }
  }
...
export function stringifyYAML(value: unknown, indentSize = 2, indentLevel = 0): string {
...
function normaliseRoom(value: string): string {
...
function isBubblePopup(stack: unknown): stack is Record<string, unknown> {
  if (!isRecord(stack)) {
    return false;
  }
...
function extractAreaFromNode(node: unknown): string | null {
  if (isRecord(node)) {
...
export function findExistingStack(
  ...
  cards.forEach((stack, index) => {
    ...
    const first = (stack as Record<string, unknown>).cards as unknown[];
    const head = first[0] as Record<string, unknown>;
    let match = false;
    if (strategy === "name") {
...
function traverse(node: unknown, visitor: (value: unknown, parent: unknown, key: string | number | null) => void, parent: unknown = null, key: string | number | null = null): void {
...
): { replaced: boolean; iconApplied: boolean } {
  let replaced = false;
      ...
      if (replacements[value]) {
        if (parent && key !== null) {
          if (Array.isArray(parent)) {
            (parent as unknown[])[key as number] = replacements[value];
          } else if (isRecord(parent)) {
            (parent as Record<string, unknown>)[key as string] = replacements[value];
          }
          replaced = true;
        }
      } else if (value === "__ICON__" && iconValue && parent && key !== null) {
        if (Array.isArray(parent)) {
          (parent as unknown[])[key as number] = iconValue;
        } else if (isRecord(parent)) {
          (parent as Record<string, unknown>)[key as string] = iconValue;
        }
        iconApplied = true;
      }
    }
...
function applyHeuristics(stack: Record<string, unknown>, room: string, areaId: string): void {
  traverse(stack, (value, parent, key) => {
    ...
    if (isRecord(parent)) {
      if (key === "area") {
        (parent as Record<string, unknown>)[key] = areaId;
      }
      if (key === "target" && isRecord(value)) {
        (value as Record<string, unknown>).area_id = areaId;
      }
    }
  });
...
export interface ApplyResult {
  ...
  replacedPlaceholders: boolean;
  iconApplied: boolean;
}

export function deepApplyTemplate(
...
export function replaceOrAppend(
  grid: Record<string, unknown>,
  index: number | null,
  stack: Record<string, unknown>,
...
export function processGrid(
  ...
  const gridCopy = deepClone(grid) as Record<string, unknown>;
  const templateCopy = deepClone(template) as Record<string, unknown>;

  const report: ProcessReportEntry[] = [];
